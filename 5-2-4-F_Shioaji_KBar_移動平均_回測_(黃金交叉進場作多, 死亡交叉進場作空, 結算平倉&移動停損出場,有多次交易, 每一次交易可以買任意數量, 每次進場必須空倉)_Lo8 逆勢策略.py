...
import numpy as np
from talib.abstract import SMA,EMA, WMA,RSI
#import sys
...
def back_test(OrderRecord, KBar_dic, LongRSIPeriod=10, ShortRSIPeriod=2, MoveStopLoss=30, Order_Quantity=1, StopLossPoint=0):
    
    # 計算 RSI 指標
    KBar_dic['RSI_long'] = RAI(KBar_dic, timeperiod=int(LongRSIPeriod))
    KBar_dic['RSI_short'] = RAI(KBar_dic, timeperiod=int(ShortRSIPeriod))
    ...
    for n in range(0, len(KBar_dic['time']) - 1):
        if not np.isnan(KBar_dic['RSI_long'][n - 1]):
            ...
            if GetOpenInterest(OrderRecord.OpenInterestQty) == 0:
                ...
                # 空單進場（黃金交叉）：short RSI 向上突破 long RSI
                if KBar_dic['RSI_short'][n - 1] <= KBar_dic['RSI_long'][n - 1] and KBar_dic['RSI_short'][n] > KBar_dic['RSI_long'][n]:
                ...
                # 多單進場（死亡交叉）：short RSI 向下跌破 long RSI
                if KBar_dic['RSI_short'][n - 1] >= KBar_dic['RSI_long'][n - 1] and KBar_dic['RSI_short'][n] < KBar_dic['RSI_long'][n]:
...
### 繪製K線圖加上MA以及下單紀錄
def ChartOrder_RSI(title, KBar_dic,TR):
    # 將K線轉為DataFrame
    Kbar_df=KbarToDf(KBar_dic)
    # 定義指標副圖
    addp=[]
    addp.append(mpf.make_addplot(Kbar_df['RSI_long'],color='red'))
    addp.append(mpf.make_addplot(Kbar_df['RSI_short'],color='yellow'))
    # 繪製指標、下單圖
    ChartOrder(title, KBar_dic,TR,addp)


#### 定義最佳化函數
def optimiztion(OrderRecord,KBar_dic,period_range_Long, period_range_Short, MoveStopLoss=10, Order_Quantity=1, isFuture='False', G_commission=0.001425):
...
LongRSIPeriod = input("請輸入RSI 長週期: ") 
ShortRSIPeriod = input("請輸入RSI 短週期: ")
CumulativeCapitalRate_series, final_return = back_test(OrderRecord, KBar_dic, LongRSIPeriod=LongRSIPeriod, ShortRSIPeriod=ShortRSIPeriod, MoveStopLoss=10, Order_Quantity=1)

#### 繪製回測相關圖: 長短移動平均線, KBar, 進場出場點位 (多空皆有)
### 畫圖: K棒, 長短移動平均線, 進出場點
#from chart import ChartOrder_RSI
ChartOrder_RSI('K棒,長短RSI交叉策略 (逆勢)', KBar_dic, GetTradeRecord(OrderRecord.TradeRecord))
...
GeneratorProfitChart(OrderRecord.Profit, StrategyName='RSI 逆勢 Strategy')

### 產出累計本利和(複利,(以1為初始資金))交易績效圖(畫出 "累計本利和清單(以1為初始資金)")
GeneratorCapitalRateChart(CumulativeCapitalRate_series, StrategyName='RSI 逆勢 Strategy')
...
#### 實際進行最佳化參數尋找 
bestcapital,bestCumulativeCapitalRate_series,bestperiod=optimizeRSI(OrderRecord, KBar_dic, np.arange(2,100,1,dtype=int), np.arange(2,100,1,dtype=int), MoveStopLoss=10, Order_Quantity=1, isFuture=isFuture, G_commission=G_commission)
#### 呈現最佳化的結果
MDD =GetMDD_CapitalRate1(bestCumulativeCapitalRate_series) 
print('最佳化的報酬率(以1為初始本金):', bestcapital-1)
print('最佳報酬率時的 MDD:', MDD)
print('最佳報酬率時的 RSI長週期:', bestperiod[0])  ## 14
print('最佳報酬率時的 RSI短週期:', bestperiod[1])  ## 10
#### 畫圖        
...
CumulativeCapitalRate_series_1, final_return_1 = back_test(OrderRecord_1, KBar_dic_TXF, LongRSIPeriod=10, ShortRSIPeriod=2, MoveStopLoss=10, Order_Quantity=1)
CumulativeCapitalRate_series_2, final_return_2 = back_test(OrderRecord_1, KBar_dic_TXF, LongRSIPeriod=10, ShortRSIPeriod=2, MoveStopLoss=10, Order_Quantity=1)
...
GeneratorProfitChart([x * 0.5 for x in Profit_list_MIX], StrategyName='RSI Strategy')

### 產出投資組合的平均累計本利和(複利,(以1為初始資金))交易績效圖(畫出投資組合的平均 "累計本利和清單(以1為初始資金)")
GeneratorCapitalRateChart([x * 0.5 for x in CumulativeCapitalRate_series_1]+ [x * 0.5 for x in CumulativeCapitalRate_series_2], StrategyName='RSI Strategy')
