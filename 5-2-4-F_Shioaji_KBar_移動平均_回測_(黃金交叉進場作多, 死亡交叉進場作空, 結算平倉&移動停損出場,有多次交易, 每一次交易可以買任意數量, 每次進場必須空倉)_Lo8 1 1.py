...
def back_test(OrderRecord, KBar_dic, LongRSIPeriod=10, ShortRSIPeriod=2, MoveStopLoss=10, Order_Quantity=1,StopLossPoint=0):
    
    # 回測取報價物件
    KBar_dic['RSI_long']=SRAI(KBar_dic,timeperiod=int(LongRSIPeriod))
    KBar_dic['RSI_short']=SRAI(KBar_dic,timeperiod=int(ShortRSIPeriod))
    ...
    for n in range(0,len(KBar_dic['time'])-1):
        ...
        if not np.isnan( KBar_dic['RSI_long'][n-1] ) :
            ...
            if GetOpenInterest(OrderRecord.OpenInterestQty)==0 :
                #Order_Quantity=1
                # 多單進場: 黃金交叉: short RSI 向上突破 long RSI
                if KBar_dic['RSI_short'][n-1] <= KBar_dic['RSI_long'][n-1] and KBar_dic['RSI_short'][n] > KBar_dic['RSI_long'][n] :
                    ...
                    continue
                # 空單進場:死亡交叉: short RSI 向下突破 long RSI
                if KBar_dic['RSI_short'][n-1] >= KBar_dic['RSI_long'][n-1] and KBar_dic['RSI_short'][n] < KBar_dic['RSI_long'][n] :
                    #Order_Quantity=1
...
#### 定義最佳化函數
def optimizeRSI(OrderRecord,KBar_dic,period_range_Long, period_range_Short, MoveStopLoss=10, Order_Quantity=1, isFuture='False', G_commission=0.001425):
...
ChartOrder_RSI('K棒,長短移動平均線,進出場點', KBar_dic, GetTradeRecord(OrderRecord.TradeRecord))
...
GeneratorProfitChart(OrderRecord.Profit, StrategyName='RSI Strategy')

### 產出累計本利和(複利,(以1為初始資金))交易績效圖(畫出 "累計本利和清單(以1為初始資金)")
GeneratorCapitalRateChart(CumulativeCapitalRate_series, StrategyName='RSI Strategy')
